...
{
    return {{IdRole, "id"}, {FileNameRole, "fileName"}, {DurationRole, "duration"}, {FileSizeRole, "fileSize"}};
}
...
void MediaItemModel::addMedia(const QString &filePath)
{
    ...
    QStringList supported = {"mp4", "mov", "avi", "mkv", "flv", "wmv", "ts", "mp3", "wav", "aac", "flac", "ogg"};
    ...
    connect(loader, &VideoThumbnailLoader::thumbnailReady, this, &MediaItemModel::onThumbnailReady);
    connect(loader, &VideoThumbnailLoader::thumbnailReady, loader, &QObject::deleteLater);
...
void MediaItemModel::onThumbnailReady(const QString &filePath, const QString &imagePath, const QString &duration)
{
    QFileInfo info(filePath);
    ...
    item.thumbnail = QImage(imagePath);
